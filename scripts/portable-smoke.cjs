...
(async()=>{
 const {circuitFixture}=await import('../tests/fixtures/circuits.js'),{programFixture}=await import('../tests/fixtures/programs.js'),{deviceFixture}=await import('../tests/fixtures/devices.js'),{halCircuit}=await import('../tests/fixtures/hal.js');
 ...
 try{
   ...
   assert.equal(await page.locator('[data-example],#extra-example,#feature-example').count(),0);assert.equal(await page.locator('#firmware-mode').inputValue(),'hal');assert.match(await page.locator('#connection-count').textContent(),/부품 0개 · 배선 0개/);await chooseHal(page,'language');await page.click('#run');await sleep(120);assert.match(await page.locator('#console-output').textContent(),/average=25/);await page.click('#run');await openFixture(page,circuitFixture());
   ...
   await page.click('#run');await openFixture(page,deviceFixture('ultrasonic'));await page.click('#run');await sleep(150);assert.match(await page.locator('#console-output').innerText(),/100/);assert.match(await page.locator('#run-status').innerText(),/실행 중/);
   const feature=async type=>{await page.click('#run');await openFixture(page,programFixture(type));await page.click('#clear-console');await page.click('#run');await sleep(180);assert.match(await page.locator('#run-status').innerText(),/실행 중/);};
   ...
   await page.click('#run');await openFixture(page,halCircuit('uart'));await chooseHal(page,'uart');await page.click('[data-monitor="bus"]');await page.click('#run');await sleep(120);await page.fill('#serial-input','Portable HAL');await page.click('#serial-send');await sleep(180);assert.match(await page.locator('#uart-output').textContent(),/Portable HAL/);assert.match(await page.locator('#console-output').textContent(),/\[USART2 TX\] Portable HAL/);assert.match(await page.locator('#console-output').textContent(),/\[USART2 RX\] Portable HAL/);assert.match(await page.locator('#run-status').textContent(),/실행 중/);await page.click('#run');await page.click('#pinout-open');assert.equal(await page.locator('[data-peripheral="USART2"]').isChecked(),true);await page.screenshot({path:path.join(out,'portable-hal-pinout.png')});await page.click('#pinout-close');assert.deepEqual(errors,[]);
   await fs.writeFile(path.join(out,'portable-smoke.json'),JSON.stringify({passed:true,exe,url:page.url(),checks:['blank HAL startup','HAL-only example catalog without circuit presets','printf output','HAL USART2 RX/TX logs','portable extraction and launch','400 interactive breadboard holes','16 part types','selected search pin','LED simulation runs','HC-SR04 example runs','LCD live text','UART round trip','I2C/SPI memory read','language example','ADC DMA completion','four-channel PWM/RC waveform'],errors},null,2));
