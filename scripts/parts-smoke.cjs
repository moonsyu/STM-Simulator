...
(async()=>{
 ...
 assert.equal(await page.locator('[data-add]').count(),16);assert.equal(await page.locator('#extra-example,#extra-parts').count(),0);assert.equal((await visibleParts()).length,16);assert.equal(await page.locator('#part-count').textContent(),'16 / 16');
 ...
 for(const [query,expected] of [['저항',['resistor','potentiometer']],['ReSiStOr',['resistor','potentiometer']],['LeD',['led','rgb']],['TMP36',['temperature']],['  HC-SR04  ',['ultrasonic']],['I2C',['i2c']],['16핀',['lcd']],['공통 음극',['rgb','sevenseg']]]){
 ...
 await page.fill('#part-search','없는 부품 xyz');assert.deepEqual(await visibleParts(),[]);assert.equal(await page.locator('#part-no-results').isVisible(),true);assert.equal(await page.locator('#part-count').textContent(),'0 / 16');
 await page.fill('#part-search','  ');assert.equal((await visibleParts()).length,16);assert.equal(await page.locator('#part-no-results').isVisible(),false);await page.fill('#part-search','');
 ...
 await load('potentiometer');await selectDemo();await page.click('#run');await page.waitForTimeout(80);assert.match(await page.locator('#console-output').innerText(),/204[78]/);await page.fill('#part-search','uart');assert.equal(await page.locator('[data-add="uart"]').isVisible(),true);assert.equal(await page.locator('[data-add="uart"]').isDisabled(),true);await page.fill('#part-search','');assert.equal(await page.locator('[data-add]:disabled').count(),16);
