...
(async()=>{
 ...
 assert.equal(await page.locator('[data-add]').count(),17);assert.equal(await page.locator('#extra-example,#extra-parts').count(),0);assert.equal((await visibleParts()).length,17);assert.equal(await page.locator('#part-count').textContent(),'17 / 17');
 ...
 await page.fill('#part-search','없는 부품 xyz');assert.deepEqual(await visibleParts(),[]);assert.equal(await page.locator('#part-no-results').isVisible(),true);assert.equal(await page.locator('#part-count').textContent(),'0 / 17');
 await page.fill('#part-search','  ');assert.equal((await visibleParts()).length,17);assert.equal(await page.locator('#part-no-results').isVisible(),false);await page.fill('#part-search','');
 ...
 await load('potentiometer');await selectDemo();await page.click('#run');await page.waitForTimeout(80);assert.match(await page.locator('#console-output').innerText(),/204[78]/);await page.fill('#part-search','uart');assert.equal(await page.locator('[data-add="uart"]').isVisible(),true);assert.equal(await page.locator('[data-add="uart"]').isDisabled(),true);await page.fill('#part-search','');assert.equal(await page.locator('[data-add]:disabled').count(),17);
