...
import {halCircuit} from './fixtures/hal.js';
...
test('every shipped example uses HAL C and contains code/configuration only',()=>{
  for(const kind of Object.keys(HAL_EXAMPLES)){const p=validateProject(halExample(kind));assert.deepEqual(p.components,[]);assert.deepEqual(p.wires,[]);assert.equal(p.firmware.mode,'hal');assert.ok(HAL_GUIDES[kind]);assert.doesNotMatch(p.code,/\bSerial\d*\.|\bsetup\s*\(|\bloop\s*\(/);const s=new SimulationSession(p);s.tick(0);s.tick(150);assert.equal(s.result.fault,false,kind);}
  for(const file of ['src/feature-examples.js','src/component-examples.js'])assert.equal(fs.existsSync(file),false);
});
test('applying a code example preserves user circuit, document name and simulation settings',()=>{
  const p=halCircuit('uart');p.name='user circuit';p.simulation={stepMs:.5,pwmWaveform:true};p.firmware.files=[{name:'old.h',text:'// old source'}];const before=structuredClone(p),next=applyHalExample(p,'adc');
  assert.deepEqual(p,before);assert.deepEqual(next.components,p.components);assert.deepEqual(next.wires,p.wires);assert.equal(next.name,p.name);assert.deepEqual(next.simulation,p.simulation);assert.notEqual(next.code,p.code);assert.deepEqual(next.firmware.files,[]);next.components[0].name='changed';assert.notEqual(next.components[0].name,p.components[0].name);
});
...
test('HAL UART transmit logging works without a terminal but does not fabricate received bytes',()=>{
  const p=halExample('uart'),events=[],s=new SimulationSession(p,{uart:e=>events.push(e)});s.tick(0);s.tick(30);assert.equal(s.inject('not wired','USART2'),0);assert.equal(s.buses.received.size,0);
...
test('HAL UART traces timed RX and IT echo with port identity',()=>{
  const events=[],s=new SimulationSession(halCircuit('uart'),{uart:e=>events.push(e)});s.tick(0);s.tick(30);s.inject('OK','USART2');s.tick(30.5);assert.equal(events.filter(e=>e.direction==='RX').length,0);s.tick(40);
...
test('converted sensor and sampling examples produce measured HAL log values',()=>{
  for(const [kind,pattern]of [['temperature',/TMP36=25.0 C/],['samples',/sample\[7\]=204[78]/],['ultrasonic',/distance=100.0 cm/],['language',/average=25/]]){const output=[],s=new SimulationSession(halCircuit(kind),{print:x=>output.push(x)});s.tick(0);for(let t=5;t<=120;t+=5)s.tick(t);assert.match(output.join('\n'),pattern,kind);}
  const p=halCircuit('ultrasonic');p.wires=p.wires.filter(w=>w.to!=='part:demo:p1');const output=[],s=new SimulationSession(p,{print:x=>output.push(x)});s.tick(0);s.tick(100);assert.ok(output.includes('HC-SR04 timeout'));
});
