...
현재 버전: **0.8.0**. 정리일: **2026-09-07**. NUCLEO-F446RE와 빵판을 연결하고 GPIO 스케치 또는 지원 HAL C 소스를 실행하는 Windows 회로 실습 앱이다. 사용자에게는 한국어 존댓말로 응답한다.
...
- 부품 검색창에서 이름·종류로 찾고, 비어 있으면 16종 전체를 세로 스크롤 목록에 표시한다.
