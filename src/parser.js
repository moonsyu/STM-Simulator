...
export function compile(source) {
  ...
  while(pos<source.length){
    ...
    if(tokens.length>20000)throw new Error('코드 토큰 수가 너무 많습니다.');
  }
  tokens.push({v:'<end>',line});let at=0,depth=0;
  const program={globals:[],functions:Object.create(null),structs:Object.create(null)};
  ...
  const isType=()=>TYPES.has(peek())||QUALIFIERS.has(peek())||peek()==='struct'||Object.hasOwn(program.structs,peek());
  function spec(){
    let constant=false;while(QUALIFIERS.has(peek())){if(take()==='const')constant=true;}
    let type;
    if(peek()==='struct'){take();type=identifier();if(!Object.hasOwn(program.structs,type))fail('정의되지 않은 구조체입니다.');}
    else {if(!isType())fail('자료형이 필요합니다.');type=take();while(['long','short','int','unsigned','signed','char','double'].includes(peek()))type=take();}
  ...
  function expression(min=0){
    ...
    if(['!','~','-','+','*','&','++','--'].includes(t))node={type:'unary',op:t,a:expression(13)};
      ...
      if(TYPES.has(peek())&&peek(1)===')'){const to=take();take();node={type:'cast',to,a:expression(13)};}
      else {node=expression();expect(')');}
    }
    else if(/^\d|^\.\d/.test(t)){const raw=(/^0[xb]/i.test(t)?t.replace(/[uUlL]+$/,''):t.replace(/[uUlLfF]+$/,''));node={type:'literal',value:Number(raw)};if(!Number.isFinite(node.value))fail('숫자 형식이 잘못되었습니다.');}
  ...
  if(!program.functions.loop)fail('void loop() 함수를 작성하세요.');
