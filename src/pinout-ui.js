...
export class PinoutPanel {
  ...
  render(){
    ...
    $('pinout-peripherals').innerHTML=Object.keys(ROUTES).map(id=>{
      const c=config.peripherals[id]||{enabled:false},fields=isSerial(id)?[['baud','Baud rate',115200]]:id==='I2C1'||id==='SPI1'?[['clock','Clock (Hz)',id==='I2C1'?100000:1000000]]:id==='TIM2'?[['prescaler','Prescaler',8399],['period','Period (ARR)',999]]:[];
      ...
      return `<section class="peripheral-card"><label class="check-label"><input type="checkbox" data-peripheral="${id}" ${c.enabled?'checked':''}>${id}</label>${routing}${fields.map(([key,label,value])=>`<label>${label}<input type="number" data-peripheral-value="${id}" data-key="${key}" value="${c[key]??value}" min="0"></label>`).join('')}</section>`;
    }).join('');
